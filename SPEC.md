...
- **Event-details Équipe tab – message déclinés compact et liste sur demande**
  - In the event-details **Équipe** (Composition) tab, when some people have declined and there are empty slots, the current long warning and always-visible "Personnes ayant décliné" section take too much vertical space. **Evolution:** Use a shorter message and show the declined list only on demand.
  - **Banner message (replace current):** Display: "Certaines personnes ont décliné ([voir qui]). Vous pouvez cliquer sur les slots vides pour choisir des remplaçants ou cliquer sur le bouton "Remplir" pour attribuer les places vacantes automatiquement." The text **[voir qui]** is a clickable link (or text button).
  - **"Personnes ayant décliné" section:** By default, do **not** display it. When the user clicks "voir qui", show the section (heading "Personnes ayant décliné", subtitle "(ne comptent pas dans la composition)", list of declined players with avatars, roles, and "Remettre en composition" button where applicable) below the banner. A second click on "voir qui" (or an equivalent control such as "voir moins") may hide the list again; exact UX (toggle vs "voir moins") can be chosen at implementation time.
  - **Scope:** Same display conditions as today: banner (and optional list) only when composition is validated by the organizer, there are declined players, and there are empty slots (`isSelectionConfirmedByOrganizer && hasDeclinedPlayers && hasEmptySlots` for the banner; `hasDeclinedPlayers` for the ability to show the list). No change to business behaviour (fill, slot click, move back to composition); only the message text, the referenced button label ("Remplir" is already used in the UI), and conditional display of the declined list.
  - **Code reference:** [src/components/SelectionModal.vue](src/components/SelectionModal.vue) (lines ~299–354: banner and declined section).
