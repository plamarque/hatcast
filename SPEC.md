...
1. **Members and troupe baseline roles (V2)** — Troupe administrators manage people linked to the troupe via **`troupe_memberships`**: activation status and **baseline troupe role**. Normative roles: **`MEMBER`**, **`TROUPE_ADMIN`**, **`EXTERNE`** (French UI on **Membres** admin: *Membre*, *Administrateur·ice de troupe*, *Externe* — one role chip dropdown, same control as member ↔ admin promotion). **`MEMBER` ↔ `EXTERNE`** conversion is bidirectional on the **same** membership row; it must preserve **season participation mode** on past seasons and, when converting to externe, let the admin choose treatment of each **active season** roster row (default: *externe saison*). **`TROUPE_ADMIN` → `EXTERNE`** is not direct — demote to **Membre** first (last-active-admin invariant). CSV import and PATCH member must not silently change baseline role to/from **`EXTERNE`**; the dedicated conversion flow applies. **`EXTERNE`** carnet entries require a display name; HatCast account and email are optional. Season/event **organizer delegation** remains separate from baseline role (DOMAIN). See [ADR 0021](docs/adr/0021-troupe-externes-carnet-invitations.md), [ADR 0022](docs/adr/0022-season-participation-mode-role-lifecycle.md).
2. **Spectacles** — manage **spectacles** (events: lifecycle, metadata, attachment to a season) for the relevant scope.
3. **Troupe and seasons** — manage **troupe** identity and **seasons**; **zero or more seasons may be active** within a troupe at the same time — activation and archiving are **explicit** per season (see DOMAIN invariant).
...
Normative intent for organizers with **`canManageComposition`** (UI chrome in [_ux-design-share-announce-manual-only.md_](_bmad-output/planning-artifacts/ux-design-share-announce-manual-only.md) — story **6.23** ; DOMAIN glossary). Applies to the Angular + Spring V2 stack.

### Purpose

Give organizers **transparent control** over manual event communication: who was **already** reached by HatCast automatic notifications, who **still needs** to be contacted manually, and a **personalized message** to share via **Copier** or **WhatsApp**. The web app does **not** bulk-send notifications from the share dialog (story **6.23**).

### Availability announcement vs reminder (single path)

- **Availability announcement** and **availability reminder** are the **same capability** from the organizer’s perspective: open the share dialog, edit the message, then **Copier** or **WhatsApp**.
- **Reminder** = any **subsequent** outreach of the availability-request family on the same event after at least one prior send of that family (including the automatic notification at publication, when it occurs).
...
- **Automatic send at publication** (`AVAILABILITY_OPENED` when availability opens) is the **first** system-initiated availability notification; it counts toward “already notified” for transparency but does not replace the manual share dialog for custom copy / WhatsApp.

**Recipient sets (decided 2026-06-04):** **Annoncer** = full active season roster; **Relance dispos** = participants with **`unknown`** availability only. Two menu entries (UX D12) — see DOMAIN.md and GET mapping in [_tech-spec-share-announce-transparency-6-17.md_](_bmad-output/planning-artifacts/tech-spec-share-announce-transparency-6-17.md) (API ; UI superseded by **6.23**).

### Other send families (same transparency rules)

Draw share, composition announce, event announce, and availability nudge follow the **same organizer transparency model** for GET `share-recipients`:

- Per-recipient **channel eligibility** (email / push / none → manual contact).
- Per-channel **notified** state when a successful delivery is logged (aggregated in UI as *already notified* vs *to reach*).
- **`lastNotifiedAt`** per channel in API responses — **not displayed** in the share dialog UI (story **6.23**); used for delivery-log semantics only.

### Recipient line (organizer UI — story 6.23)

When GET `share-recipients` succeeds, a **single compact line** appears below Copier / WhatsApp:

- **Given** `alreadyCount > 0` and `toReachCount > 0`, **when** the line is shown, **then** a link-style control shows *N personne(s)* (menu with names) + *déjà notifiées automatiquement. Reste à prévenir :* + name chips.
- **Given** `alreadyCount === 0`, **when** the line is shown, **then** *Reste à prévenir :* + chips only.
- **Given** `toReachCount === 0`, **when** the line is shown, **then** the link + *déjà notifiées automatiquement.* only (no chips).
- **Given** a participant joined the roster **after** an earlier automatic send, **when** the line is shown, **then** they appear in *to reach* until a future HatCast delivery logs them as notified.

### Notified semantics (normative)

- **Already notified** (UI aggregate): `channels.email.notified || channels.push.notified` for the current dialog intent’s mapped delivery-log intents.
- **Notified** on a channel means at least one **`notification_delivery_log`** row for that `(event_id, user_id, channel)` with status **`SENT` or `PARTIAL`** for the notification intent(s) mapped to the current dialog intent — including automatic publication and (when logged) API manual POST sends, not only automatic publication.
...
- **Inline composition in event-details Composition tab (no separate popup)**
  ...
  - **Immediate replay draw factor (Story 19.9, optional):** When enabled in a custom draw formula (not in production DEFAULT until **19.16**), the draw may **exclude or heavily penalize** candidates who held the **same role** on the immediate validated predecessor in the same compartment — same trigger as the 6.20 warning, but affects **weights** only (manual assign still allowed). See DOMAIN.md and [draw-weight-engine-v1-spec.md](docs/v2/technical/draw-weight-engine-v1-spec.md).
  - **Role request aspiration bonus (Story 19.10, optional):** When enabled in a custom draw formula (not in production DEFAULT until **19.16**), the draw **boosts** candidates who repeatedly declared availability for a role on past **validated** shows in the same season/compartment without ever being **selected** for that role (`unfulfilledRoleRequestCount`). Bonus only — no malus for frequent requesters. See DOMAIN.md and [draw-weight-engine-v1-spec.md](docs/v2/technical/draw-weight-engine-v1-spec.md).
...
- **Factor coefficients:** Troupe admins may tune **factor coefficients** (intensity per criterion — malus/bonus params), not only enable/disable toggles; normative param catalogue in [draw-formulas-policies-spec.md § Factor catalogue](docs/v2/technical/draw-formulas-policies-spec.md#factor-catalogue) (**19.19a**); runtime **19.19b**, admin UI **19.19c**.
...
- **Effective formula in Équipe UI (19.21, PO 2026-06-19):** Server applies **`effectiveFormulaId` by default** — **no** bandeau, chip, or pre-draw dialog. **`MANDATORY`** or **CHOICE with one** allowed formula → **no** formula control on Équipe. **`CHOICE` with ≥2** allowed formulas → optional change **only** in the existing **`more_vert`** overflow menu (section « Formule de tirage »). **Tirer au sort** is immediate; client sends `formulaId` matching the session selection. **`%` and breakdown follow `selectedFormulaId`** (**OQ-19-04** — runtime **19.18**). UX authority: [_ux-design-orga-formula-choice-19-21.md](_bmad-output/planning-artifacts/ux-design-orga-formula-choice-19-21.md). No per-event **policy** row — only session-scoped formula selection.
...
- **Troupe admins** configure policies (**19.18** / UI **19.20** later); season organizers consume effective rules; formula change at draw is rare and optional in UI when ≥2 formulas allowed.
