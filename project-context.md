...
7. **Worktree par story** — Chaque unité BMad se développe dans un worktree adjacent dédié sur `feat/{story-key}` (ex. `feat/17-43-event-detail-contexte-infos`), créé depuis `origin/v2` avec `./scripts/v2/story-branch.sh start {story-key}`. **`v2`** = intégration / deploy dev cloud propre, jamais un checkout de travail story. Le bootstrap installe et vérifie les workflows BMad standards épinglés dans cette seule unité; il demande l’accord explicite `HATCAST_BMAD_ALLOW_NETWORK=1` si un téléchargement est nécessaire. Après revue humaine approuvée, l’invocation explicite `./scripts/v2/story-branch.sh integrate {story-key}` merge, pousse `v2`, vérifie le remote, puis supprime l’unité et la branche locale; elle ne supprime jamais la branche distante de story. Ne jamais copier de secrets ni enregistrer un chemin local dans le frontmatter. Détail : [`_bmad/custom/story-branch-workflow.md`](_bmad/custom/story-branch-workflow.md). Skills BMad surchargés : `bmad-create-story`, `bmad-dev-story`, `bmad-code-review`.
