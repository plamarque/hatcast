...
- **Context:**
  ...
  - **Development** relies on Flyway **seed** data (troupe La Malice fictive, spectacles `[MVP]`, etc.) for local recette and integration tests.
...
- **Decision:**
  1. **Flyway split:** `classpath:db/migration` = schema and reference data only; `classpath:db/seed` = dev/test seed SQL (La Malice, MVP pilot, etc.).
  2. **Profiles:**
  ...
  4. **Pre-prod content:** After deploy, Postgres staging contains **only** schema + data imported via documented CSV/API migration (today: users + troupe members per [v1-troupe-members-csv-recipe.md](../v2/migration/v1-troupe-members-csv-recipe.md)). No `@seed.la-malice.test` accounts unless manually imported.
