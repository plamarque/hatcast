...
- **Decision:**
  ...
  4. **CI/CD:** Les secrets **`HATCAST_DATASOURCE_*`** et **`HATCAST_CORS_ALLOWED_ORIGINS`** sont stockés par **environnement GitHub** (`development`, `staging`, `production`) et reliés aux branches git **`v2`**, **`staging-v2`**, **`production-v2`** respectivement (la branche git **`staging`** reste V1 / Firebase ; **`main`** reste la prod V1 Firebase — voir [`docs/v2/technical/DEPLOY_V2_CLOUD_RUN.md`](../v2/technical/DEPLOY_V2_CLOUD_RUN.md) et [`DEPLOYMENT_WORKFLOW.md`](../v2/technical/DEPLOYMENT_WORKFLOW.md)).
