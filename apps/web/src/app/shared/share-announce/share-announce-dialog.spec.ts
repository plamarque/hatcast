...
import * as shareMessages from '../../core/messaging/share-announce-messages'
...
describe('ShareAnnounceDialog', () => {
  ...
  it('opens WhatsApp with edited message text', async () => {
    ...
    component.messageText = edited
    const urlSpy = vi.spyOn(shareMessages, 'buildWhatsAppSendUrl')

    component['openWhatsApp']()
    expect(urlSpy).toHaveBeenCalledWith(edited)
    urlSpy.mockRestore()
  })
