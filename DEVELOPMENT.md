...
**Web Push (V2, story 8.1)** : renseigner `HATCAST_WEB_PUSH_VAPID_PUBLIC_KEY` dans `.env` (même clé publique que V1). L’API l’expose via `GET /v1/config/public` ; le SPA s’en sert pour `PushManager.subscribe`. Clé privée (`HATCAST_WEB_PUSH_VAPID_PRIVATE_KEY`) : story 8.3 uniquement — voir [DEPLOY_V2_CLOUD_RUN.md](docs/v2/technical/DEPLOY_V2_CLOUD_RUN.md).
