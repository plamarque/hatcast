...
- **Audit V2 (write path, Story 9.0) :** migration `V43__audit_events.sql` ; module `com.hatcast.api.audit` (`AuditEventRecorder`, enum `AuditActionType`) ; hooks dans les services domaine (dispos, événements, rosters, droits, composition, participation). Écriture **dans la même transaction** que la mutation (contraste avec notifications after-commit, story 6-13). Consultation : stories **9.1** / **9.2** (post-MEP). Voir [ADR-0018](docs/adr/0018-v2-audit-events-postgres.md).
...
| **Audit (V2 API write path)** | Append-only `audit_events` journal on domain mutations (FR35) | [services/api/src/main/kotlin/com/hatcast/api/audit/](services/api/src/main/kotlin/com/hatcast/api/audit/) — [ADR-0018](docs/adr/0018-v2-audit-events-postgres.md) |
