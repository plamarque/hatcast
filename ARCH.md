...
- **Durée de session V2 (BUG-014) :** l’intervalle serveur JDBC est une limite d’inactivité glissante (`30 jours` si `rememberMe=true`, sinon `30 minutes`). La décision est conservée comme booléen dans la session serveur afin que toute réémission de `HATCAST_SESSION` garde la même politique. Pour une connexion mémorisée, le cookie a un `Max-Age` de 30 jours à compter de son émission ou réémission ; le produit restaure donc la session à la réouverture si ce cookie est encore valable dans cette fenêtre. Sans mémorisation, `HATCAST_SESSION` est un cookie de navigateur, sans `Max-Age` ni `Expires` ; un navigateur peut néanmoins restaurer son état de session après redémarrage. Il est toujours `HttpOnly`, `SameSite=Lax` et, en profil Cloud, `Secure`. Le client ne conserve qu’une préférence booléenne : après perte du cookie, seul un utilisateur Identity Platform/Firebase déjà présent peut rééchanger un ID token ; le flux Google GIS ne crée pas de session Firebase et redemande une connexion. Les abonnements Web Push Postgres sont lus par le dispatcher, indépendamment de toute session HTTP active.
...
- **Contrat membres V2 :** `services/api/openapi/seasons.yaml` documente `/v1/troupes/{troupeId}/members*`, l’export/import CSV admin (`/members/export`, `/members/import`), la désactivation soft (`status = INACTIVE`), la garde dernier admin actif, les exigences CSRF et la limite demo self-join (`MEMBER` uniquement). **Conversion lifecycle :** `GET …/members/{id}/conversion-context`, `POST …/convert-to-externe`, `POST …/convert-to-member` (PATCH/CSV refusent le flip silencieux vers/depuis `EXTERNE`). Contrat CSV v1 : `_bmad-output/implementation-artifacts/2-3-import-export-csv-des-membres-de-troupe.md`. **Externes troupe (carnet, scope invitation, cascade à l’ajout) :** [ADR-0021](docs/adr/0021-troupe-externes-carnet-invitations.md) — livré (stories **2.21**, **3.23**, **3.25**). **Mode participation saison + lifecycle membre ↔ externe :** [ADR-0022](docs/adr/0022-season-participation-mode-role-lifecycle.md) — livré (story **2.26**).
...
- **Availability reminder dispatch:** `share-recipients` builds a preference-aware preview for `availability_nudge`; its confirmation fingerprint is required by POST, which revalidates it before using the existing email/push dispatcher. The response records request acceptance, not provider delivery.
