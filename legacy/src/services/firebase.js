...
import { mapGoogleSignInError, googleSignInGenericCleanError } from '../utils/googleSignInErrors.js'
...
export async function signInWithGoogle() {
  try {
    ...
    const mapped = mapGoogleSignInError(error)
    if (mapped) {
      const cleanError = new Error(mapped.message)
      cleanError.code = mapped.code
      cleanError.isAuthError = mapped.isAuthError
      throw cleanError
    }
    ...
    const gen = googleSignInGenericCleanError()
    const cleanError = new Error(gen.message)
    cleanError.code = gen.code
    cleanError.isAuthError = gen.isAuthError
    throw cleanError
  }
}
