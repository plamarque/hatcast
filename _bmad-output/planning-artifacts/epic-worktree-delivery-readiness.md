...
| 6 | `21-6-github-review-decision-controller` | Bind a story worktree to its GitHub PR, exact head SHA, Patrice's attested decision, and one delivery lane before the existing integration command may run. | Existing `story-branch.sh integrate` contract; independent of 21-4 and 21-5 completion | Hermetic fixtures prove stale, ambiguous, missing-attestation and changed-head states fail closed. |
...
3. **Given** the human review decision is valid, **when** integration is requested, **then** the existing `integrate` command alone preserves push, remote-ancestry and recoverable-cleanup guards.
...
2. **Given** Patrice directly sets the lane and head-SHA attestation in GitHub, or explicitly decides `batch` or `release-now` in conversation, **when** the controller inspects or records the decision, **then** it binds the lane and attestation to the current PR head SHA without submitting a GitHub `APPROVED` review.
3. **Given** requested changes, unresolved blocking comments, an invalid PR state, or a missing or stale Patrice attestation, **when** integration is requested, **then** the controller fails closed and leaves the unit available for repair.
...
| G6 GitHub review/integration | Patrice then machine | current PR head, Patrice attestation, exactly one lane label, no blocking review state; existing `integrate` guards | Preserve unit on failure. |
