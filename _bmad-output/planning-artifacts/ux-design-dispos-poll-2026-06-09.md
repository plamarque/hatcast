...
| **D12** | **Cotes % (explainability)** : **pas** sur la jauge D8. Si `explainabilityEnabled` et données présentes, les `%` restent **dans le pool déplié** (segments interactifs + breakdown sheet — comportement Tous actuel). **Gate :** `explainabilityEnabled` quand le **spectacle est publié** (dispos ouvertes — **3.21**), **pas** quand la composition est validée. L’état brouillon de composition **n’affecte pas** Dispos. |
