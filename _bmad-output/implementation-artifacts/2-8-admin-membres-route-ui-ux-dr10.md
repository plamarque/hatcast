...
4a. **Given** an active troupe member row, **when** the admin clicks **Retirer**, **then** a confirmation dialog explains that the member will be removed from the troupe and lose associated access, but the HatCast user account will not be deleted; confirming calls the existing soft-deactivation/removal endpoint, hides the member from the default active list, and preserves last-active-admin protection — **FR7**.
...
- [x] **Membres tab** (AC: 4, 6, 7, 8)
  ...
  - [x] Add explicit **Retirer** action with confirmation copy stating the HatCast user account is not deleted; call existing soft-deactivation endpoint and refresh members/permissions.
