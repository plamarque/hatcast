...
| Métrique | Valeur |
|----------|--------|
| Entrées actives | **11** (DW-108–125, hors fermés) |
| **T0** | **0** |
| **T1** | **0** |
| **T2** (avant M4) | **4** — ordre : DW-118 → DW-119 → DW-109 → DW-110 |
| **T3** (backlog) | **9** actionnables + **DW-108** accepté |
| Fermés cette passe | **DW-105**, **DW-106**, **DW-114** |
...
|---|-----|------|----------|--------|--------|--------|
| 1 | **DW-118** | **S** | Modéré | Faible | N | SQL backfill `removal_source` |
| 2 | **DW-119** | **S** | Modéré | Faible | N | Vérif slugs MIG-2 |
| 3 | **DW-109** | **M** | Élevé si trigger | Élevé si trigger | M | Rejects transform MIG-3 |
| 4 | **DW-110** | **M** | Modéré | Modéré | M | Doc / purge orphelines MIG-3 |
...
1. **T2** (ROI décroissant) — **DW-118** → **DW-119** (quick wins migration) → **DW-109** (défense données) → **DW-110** (staging).
