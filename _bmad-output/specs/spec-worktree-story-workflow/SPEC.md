...
| D-2 | `scripts/v2/bmad-runtime.env` pins `bmad-method` and the installed modules. The bootstrap invokes that installer only for the requested unit, after explicit network confirmation when its cache is insufficient, then verifies the standard `bmad-create-story`, `bmad-dev-story`, and `bmad-code-review` skills together with tracked BMad scripts and policy. Tests inject a local provisioner; it never copies secrets, arbitrary ignored data, or another checkout. The live pinned-installer path still requires an explicit operator run. |
