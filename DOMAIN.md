...
- **Availability:** A player's status for an event (e.g. available / unavailable). Stored per player per event in V1 (e.g. `availability` subcollection or nested; see `playerAvailabilityService.js`, `storage.js`). **V2 (PostgreSQL):** table `event_availability`, keyed by `(event_id, user_id)`; API status `available` | `unavailable` | `unknown` (no row). When `available`, column `role_keys` holds a JSON array of event-required role keys the member offers; a **historical empty array** retains general availability (eligible for any required role on the event, V1 parity). New self and proxy `available` writes require at least one normalized event role when roles are offered; offered volunteer is included even when no other role is selected. Missing, empty or whitespace-only roles are rejected with HTTP 400 only when roles are offered without volunteer. Events without role slots still accept general availability. Historical records are not migrated or changed on read/open/cancel. An editable available draft includes offered volunteer, marked as unsaved until explicitly submitted; optional roles are not preselected. Comment-only poll saves require an explicit role selection before replacing a historical wildcard response.
...
- **Season participation mode (V2):** Normative mode on `season_participants` for **how this person participates on this season**, independent of their **current** troupe baseline role. Values: **`MEMBER_SYNC`** (troupe member roster row — full member season workspace when membership allows); **`GUEST_SEASON`** (externe-style season invite — partial guest workspace, Laetitia MC); **`GUEST_EVENT`** (externe-style event invite — Ruben DJ). Paired with `invitation_scope` for guest modes. See [ADR 0022](docs/adr/0022-season-participation-mode-role-lifecycle.md). Historical seasons retain **`MEMBER_SYNC`** when a person is later converted to **`EXTERNE`** at troupe level.
- **Troupe role lifecycle (V2):** Organizers may convert **`MEMBER` ↔ `EXTERNE`** on the **same** `troupe_memberships` row over time (bidirectional). Troupe role drives **defaults for new seasons** (member auto-sync vs invite-only). **Per-season** behaviour is governed by **season participation mode**, not troupe role alone. HatCast account on carnet entries remains **optional**; linking enables self-service dispos on in-scope events. See [ADR 0022](docs/adr/0022-season-participation-mode-role-lifecycle.md).
...
- **Season participant roster vs troupe membership:** A **`MEMBER`** may be absent from one season’s roster while remaining an active troupe member and present on other seasons. An **`EXTERNE`** may be in the troupe carnet without any season roster row until invited. A person may be **`MEMBER_SYNC`** on a **past** season and **`GUEST_SEASON`** on a **current** season after troupe role conversion (**ADR 0022**).
- **Troupe role conversion (V2):** **`MEMBER` → `EXTERNE`:** same membership row; stop auto-sync on new seasons; admin selects active seasons to keep as **`GUEST_SEASON`** or remove from roster (via role chip → *Externe*; dialog when active seasons have roster rows); past seasons default to preserved **`MEMBER_SYNC`**. **`EXTERNE` → `MEMBER`:** same row; re-run membership sync on active seasons (respect season-admin removals); via role chip → *Membre*. **`TROUPE_ADMIN` → `EXTERNE`:** not direct — demote to **`MEMBER`** first. Forbidden: second membership row for the same `(troupe, user)` to simulate conversion; forbidden: silent PATCH/CSV role flip to/from **`EXTERNE`**.
...
## Disponibilité bénévole obligatoire (V2)
