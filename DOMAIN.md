...
- **Troupe role conversion (V2):** **`MEMBER` → `EXTERNE`:** same membership row; stop auto-sync on new seasons; admin selects active seasons to keep as **`GUEST_SEASON`** or remove from roster; past seasons default to preserved **`MEMBER_SYNC`**. **`EXTERNE` → `MEMBER`:** same row; re-run membership sync on active seasons (respect season-admin removals). Forbidden: second membership row for the same `(troupe, user)` to simulate conversion.
