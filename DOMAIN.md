...
- **Season (V2 code/DB: `season`; French UI: Saison):** A first-class programme container within a troupe: temporal bounds, stable participant roster, events, draws, and season-scoped views (Agenda, Historique, Statistiques). Has a slug (URL-safe id). A troupe may run **several active seasons concurrently** (ADR 0011). Routes: **`/saison/:slug`** (canonical); `/ligue/:slug` is a transition alias. See [ADR 0011](docs/adr/0011-league-model-and-user-agenda.md), [ADR 0013](docs/adr/0013-troupe-navigation-equity-tags-event-slugs.md).
- **League:** **Deprecated French UI label** (sounds like inter-troupe competition). Use **Saison** in new copy. Code may still say `league` in helpers until rename.
- **Equity tag (optional, per event):** A single optional label on an event (`equity_tag`) that assigns participations to a **fairness compartment** for chances, auto-draw, and related stats within a season. **Empty = principal compartment** (system default; not shown in UI). Examples: `deplacements`, `aperock`. Distinct from **event template type** (`match`, `cabaret`, …). Not navigable. See [ADR 0013](docs/adr/0013-troupe-navigation-equity-tags-event-slugs.md).
...
- **Travel league (*ligue déplacements*):** **Superseded for new work** by **equity tags** (ADR 0013). Historical ADR 0012 approach: separate season for away shows. Legacy `template_type = deplacement` and any existing travel seasons remain until migrated to `equity_tag = deplacements` on events in the primary season.
...
- **Déplacement (V2 cible) :** événements avec **`equity_tag = deplacements`** (ou équivalent troupe) → comptés en **DEPLACEMENT** uniquement pour stats/chances ; ne comptent pas dans JEU/DECORUM du compartiment principal.
- **Legacy :** événement `templateType = deplacement` → migrer vers `equity_tag` ; règles de lecture jusqu’à migration (ADR 0013).
