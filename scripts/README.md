...
- **`start-dev.sh`** : Démarre la stack **V2** en local (API Spring sur 8080 + Angular sur 4200, `ng serve --host`). **`tailscale up`** si le VPN est coupé, puis **Tailscale Serve** vers `:4200` si absent (accès mobile MagicDNS). **`--no-tailscale`** ou **`HATCAST_SKIP_TAILSCALE_SERVE=1`** pour ignorer. Option **`./scripts/start-dev.sh --legacy`** pour l’ancien flux (seul le client V1 / Vite). Charge **`.env`** à la racine via **`load-dotenv.sh`** (toutes les variables reconnues).
