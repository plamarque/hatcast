...
@ActiveProfiles("test")
class ReconfirmationNotificationIntegrationTest {
    @Autowired
    ...
    fun `unlock notifies only assignees whose response was reset`() {
        ...
        val confirmedId = participantIdForUser(seasonId, "sub-reconf-confirmed")
        val pendingId = participantIdForUser(seasonId, "sub-reconf-pending")
        seedComposition(eventId, confirmedId, pendingId)
        mockMvc
            .perform(
    ...
    fun `revalidate after unlock does not resend confirmation or FYI`() {
        ...
        seasonParticipantService.ensureMembershipParticipants(seasonRepository.findById(seasonId).orElseThrow())
        val eventId = createEvent(admin, seasonId)
        val assigneeId = participantIdForUser(seasonId, "sub-revalidate-assignee")
        seedComposition(eventId, assigneeId, null)
        mockMvc
            .perform(
                post("/v1/seasons/$seasonId/events/$eventId/composition/validate")
                    .cookie(admin)
    ...
    private fun seedComposition(
        eventId: UUID,
        slot0ParticipantId: UUID,
        slot1ParticipantId: UUID?,
    ) {
        val now = Instant.now()
        ...
        slotRepository.save(
            EventCompositionSlotEntity(
                ...
                roleKey = "player",
                slotIndex = 0,
                seasonParticipantId = slot0ParticipantId,
                participationStatus = SlotParticipationStatus.PENDING,
            ),
        )
        slot1ParticipantId?.let { participantId ->
            slotRepository.save(
                EventCompositionSlotEntity(
                    eventId = eventId,
                    roleKey = "player",
    ...
    private fun createEvent(
        ...
        val future = Instant.parse("2032-10-01T19:00:00Z")
        val res =
            mockMvc
                .perform(
