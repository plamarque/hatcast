...
@Tag("19.10")
class DrawRoleRequestGoldenTest {
    @ParameterizedTest(name = "{0}")
    @MethodSource("fixtures")
    fun roleRequestGoldenFixture(fixture: DrawGoldenFixture) {
    ...
    private fun pipelineFor(@Suppress("UNUSED_PARAMETER") fixture: DrawGoldenFixture) =
        DrawWeightPipelines.withRoleRequest()

    private fun parseUnfulfilledMap(input: com.fasterxml.jackson.databind.JsonNode): Map<java.util.UUID, Int> {
    ...
    private fun assertWeightForParticipant(fixture: DrawGoldenFixture) {
        ...
        val actual =
            AvailabilityChanceCalculator.weightForParticipant(
                ...
                unfulfilledRoleRequestCount = input.path("unfulfilledRoleRequestCount").asInt(0),
            )
    ...
    private fun assertScoreCandidates(fixture: DrawGoldenFixture) {
        ...
        val unfulfilled = parseUnfulfilledMap(input)
        val scored =
    ...
    companion object {
        ...
        fun fixtures(): List<DrawGoldenFixture> {
            val stream =
                requireNotNull(
                    DrawRoleRequestGoldenTest::class.java.classLoader.getResourceAsStream(
                        "draw/golden/role-request/bonus.json",
                    ),
                ) { "Missing role-request golden fixtures" }
