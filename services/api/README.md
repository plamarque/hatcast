...
- **Fixtures hybrides** (option C) : seed minimal + `POST /v1/e2e/fixtures/story-3-19/reset`, `POST /v1/e2e/fixtures/story-3-8d/reset` et `POST /v1/e2e/fixtures/e1-cutover/reset` (en-tête `X-Hatcast-E2E-Key`, clé par défaut `e2e-fixtures-secret` dans `application-e2e.yml`). Tokens : `e2e-admin`, `e2e-member` (Angie).
